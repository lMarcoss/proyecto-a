...
public class PagoLuzCRUD extends Conexion implements OperacionesCRUD {
    ...
    public <T> List listar(String id_jefe, String rol) throws Exception {
        ...
        if (rol.equals("Administrador")) {
            consulta = "SELECT * FROM VISTA_PAGO_LUZ WHERE id_jefe = ? ORDER BY fecha DESC";
        } else {
            consulta = "SELECT * FROM VISTA_PAGO_LUZ WHERE id_jefe = ? AND fecha = CURDATE() ORDER BY fecha DESC";
        }
        try {
            this.abrirConexion();
    ...
    public <T> List buscar(String nombre_campo, String dato, String id_jefe, String rol) throws Exception {
        ...
        String consulta;
        if (rol.equals("Administrador")) {
            consulta = "SELECT * FROM VISTA_PAGO_LUZ WHERE " + nombre_campo + " like ? AND id_jefe = ? ORDER BY fecha DESC";
        } else {
            consulta = "SELECT * FROM VISTA_PAGO_LUZ WHERE " + nombre_campo + " like ? AND id_jefe = ? AND fecha = CURDATE() ORDER BY fecha DESC";
        }
        try {
            this.abrirConexion();
            try (PreparedStatement st = this.conexion.prepareStatement(consulta)) {
